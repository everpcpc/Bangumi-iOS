...
import_profile() {
  ...
  decode_base64_to_file "$profile_base64" "$temp_profile"
  ...
  if [ -n "$uuid" ]; then
    ...
    echo "[OK] Imported $platform profile: $uuid"
  else
    local fallback_name="${platform}${file_ext}"
    cp "$temp_profile" "$PROFILES_DIR/$fallback_name"
    echo "[WARN] Imported $platform profile (UUID not found, using: $fallback_name)"
  fi
}
