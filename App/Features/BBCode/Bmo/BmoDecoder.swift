...
struct BBCodeBmoDecoder {
  private static let manifest: BmoManifest? = loadManifestFile()

  struct BmoManifest: Codable, Sendable {
    ...
    struct BmoCategory: Codable, Sendable {
      let id: String
      let name: String
      let layer: Int
      ...
      struct BmoItem: Codable, Sendable {
        let id: String
        let alias: String?
        let src: String
        let layer: Int
  ...
  private static func loadManifestFile() -> BmoManifest? {
  ...
  private static var url: URL? {
    BangumiStickerResources.url(
      forResource: "manifest.local",
      withExtension: "json",
      subdirectory: "Bmo"
    )
    ?? BangumiStickerResources.url(forResource: "manifest.local", withExtension: "json")
  }
  ...
  private static func decodeCompact(_ code: String) -> BmoResult {
    ...
    var reader = BBCodeBmoVariableLengthReader(bytes: bytes)
    ...
    while reader.hasMore() {
      ...
      guard let manifest = loadManifest(),
        let itemData = findBBCodeBmoItemByCompactId(compactId, in: manifest)
      else {
        continue
      }
...
struct BBCodeBmoVariableLengthReader {
  let bytes: [UInt8]
  var offset: Int = 0
...
private func findBBCodeBmoItemByCompactId(_ compactId: UInt32, in manifest: BBCodeBmoDecoder.BmoManifest) -> (
