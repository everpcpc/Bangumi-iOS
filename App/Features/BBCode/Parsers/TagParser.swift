...
func parseBBCodeOpeningTag(_ g: inout BBCodeScalarIterator, _ worker: BBCodeParserWorker) -> BBCodeParserState? {
  ...
  while let c = g.next() {
    if isFirst && c == UnicodeScalar("/") {
      if !worker.currentNode.paired {
        ...
        restoreBBCodeNodeToPlainText(node: newNode, c: c, worker: worker)
        return .content
      }
      ...
      newNode.value.insert(Swift.Character(UnicodeScalar(91)), at: newNode.value.startIndex)
      return .tag
    } else {
...
func restoreBBCodeNodeToPlainText(node: BBCodeNode, c: UnicodeScalar, worker: BBCodeParserWorker) {
  node.setTag(tag: worker.tagManager.getInfo(type: .plain)!)
  node.value.insert(Swift.Character(UnicodeScalar(91)), at: node.value.startIndex)
