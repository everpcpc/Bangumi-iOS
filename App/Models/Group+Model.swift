...
extension ChiiGroup {
  var creator: SlimUserDTO? {
    get { PersistedJSON.decode(SlimUserDTO.self, from: creatorData) }
    set { creatorData = newValue.flatMap(PersistedJSON.encode) }
  }

  var moderators: [GroupMemberDTO] {
    get { PersistedJSON.decode([GroupMemberDTO].self, from: moderatorsData) ?? [] }
    set { moderatorsData = PersistedJSON.encode(newValue) ?? moderatorsData }
  }

  var recentMembers: [GroupMemberDTO] {
    get { PersistedJSON.decode([GroupMemberDTO].self, from: recentMembersData) ?? [] }
    set { recentMembersData = PersistedJSON.encode(newValue) ?? recentMembersData }
  }

  var recentTopics: [TopicDTO] {
    get { PersistedJSON.decode([TopicDTO].self, from: recentTopicsData) ?? [] }
    set { recentTopicsData = PersistedJSON.encode(newValue) ?? recentTopicsData }
  }
